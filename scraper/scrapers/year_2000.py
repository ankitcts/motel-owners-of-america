"""
Year 2000 Scraper — Census County Business Patterns (CBP)

Sources:
1. Census CBP API (NAICS1997 7211 - Traveler Accommodation)
   - State + county level establishment counts, employment, payroll

Output: MongoDB collections (state_summaries, county_summaries) + JSON backup
"""
...
from utils.db import (
    ...
    start_scrape_job, finish_scrape_job, get_stats,
)
...
def scrape_cbp_state():
    """Fetch CBP 2000 state-level data for NAICS 7211."""
...
def save_to_mongodb(state_data: dict, county_data: dict):
    """Save scraped data to MongoDB with dedup tracking."""
    ...
    for s in state_data.get("data", []):
        upsert_state_summary({
            ...
            "owner_count": round(s["establishments"] * 0.7),
            "source": SOURCE,
        })
    ...
    for c in county_data.get("data", []):
        upsert_county_summary({
            ...
            "owner_count": round(c["establishments"] * 0.7),
            "source": SOURCE,
        })
        stats["records_processed"] += 1
        stats["records_inserted"] += 1
...
def save_processed_json(state_data: dict, county_data: dict):
    """Also save processed JSON for frontend (backward compat)."""
    ...
    print(f"  States: {len(processed['states'])}, Counties: {len(processed['counties'])}")
...
def main():
    ...
    try:
        ...
        print(f"  MongoDB save failed (will continue with JSON): {e}")
